...
/****** Object:  StoredProcedure [dbo].[USP_ADMINBUCKETBYTEAM]    Script Date: 04/16/2021 10:58:54 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[USP_ADMINBUCKETBYTEAM]
@TEAMID		INT
AS 
BEGIN
...
(SELECT tblFacilityAdminTeamBucket.FACILITYID, tblFacilityAdminTeamBucket.TEAMID,TeamName 
INTO #TEAMS 
FROM tblFacilityAdminTeamBucket 
INNER JOIN TblAdminTeams On TblAdminTeams.TeamID = tblFacilityAdminTeamBucket.teamID
INNER JOIN TblAdminTeamMembers on TblAdminTeamMembers.TeamID = TblAdminTeams.TeamID
INNER JOIN FacPackets On FacPackets.FacilityID = tblFacilityAdminTeamBucket.FacilityID
WHERE tblFacilityAdminTeamBucket.TEAMID = @TEAMID
GROUP BY tblFacilityAdminTeamBucket.FACILITYID, tblFacilityAdminTeamBucket.TEAMID,TeamName)
...
SELECT 	CONTACT1.COMPANY AS Facility,
		CONTACT1.[STATE], 
		CONTACT1.KEY4 AS AM, 
		CONTACT1_1.CONTACT AS Nurse, 
        CONTACT1_1.KEY4 AS Recruiter,
		FacPackets.RequestDate AS Submitted, 
		FacPackets.FacPacketID, 
        DATEDIFF(Minute, GETDATE(), FacPackets.RequestDate) AS ElaspeTime, 
		LookupValue_1.ShortDesc AS SubmitStatus, 
        FacPackets.Cancelled, 
		FacPackets.Fired, 
		FacPackets.CandidateID, 
		FacPackets.RequestID, 
		FacPackets.AssocJobID,
		case FacPackets.RegistryPrivateID when '' then jr.RegistryPrivateID 
			else FacPackets.RegistryPrivateID 
		end as RegistryPrivateID,
		FacPackets.Notes, 
		CONTACT1_1.KEY2, 
		JobSpecialty.SpecialtyName AS ShortDesc,
		...
		ISNULL(VMS.VMSPortal,'None') as VMS,
		CONTACT1.ADDRESS1,
		CONTACT1.CITY,
		CONTACT1.ZIP,
		FacPackets.AmToAdmin,
		(select ISNULL(UHLTHSYS,'') from CONTACT2 where ACCOUNTNO=CONTACT1.ACCOUNTNO) as USNDINVTO,
		ISNULL(Jr.fk_StatusID,0) as JobStatus
FROM FacPackets 
INNER JOIN CONTACT1 ON FacPackets.FacilityID = CONTACT1.id 			 
...
INNER JOIN #TEAMS T ON T.FacilityID = FacPackets.FacilityID
...
WHERE  (FacPackets.AcctManager IS NULL)  
and (ulb.branch_Id = 17 or ulb.Branch_ID is null) AND (FacPackets.SubmitStatus = 411) OR (FacPackets.SubmitStatus = 415) OR (FacPackets.SubmitStatus = 496) 
AND teamID = @TEAMID 
ORDER BY Facility

DROP TABLE #CANCELLED,#TEAMS,#TEMP,#VMSPORTAL 
END
