...
/****** Object:  StoredProcedure [dbo].[USP_ADMINBUCKETBA]    Script Date: 04/16/2021 10:57:14 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[USP_ADMINBUCKETBA]
AS 
BEGIN
...
SELECT CONTACT1.COMPANY AS Facility,CONTACT1.[STATE], CONTACT1.KEY4 AS AM, CONTACT1_1.CONTACT AS Nurse,
                 ...
                 DATEDIFF(Minute, GETDATE(), FacPackets.RequestDate) AS ElaspeTime, LookupValue_1.ShortDesc AS SubmitStatus,
                  FacPackets.Cancelled, FacPackets.Fired, FacPackets.CandidateID, FacPackets.RequestID, FacPackets.AssocJobID,
                   FacPackets.RegistryPrivateID,FacPackets.Notes, CONTACT1_1.KEY2, JobSpecialty.SpecialtyName AS ShortDesc,
				...
				ISNULL(Jr.fk_StatusID,0) as JobStatus
                   ...
                   INNER JOIN userlistBranch ulb on ul.EsigId = ulb.EsigId
                LEFT JOIN JobRequests JR ON JR.pk_JobRequestID = FacPackets.RequestID
                   WHERE  (FacPackets.AcctManager IS NULL) and (ulb.branch_Id = 17 and ulb.Active =1) AND (FacPackets.SubmitStatus = 411) OR (FacPackets.SubmitStatus = 415)
                   OR (FacPackets.SubmitStatus = 496) ORDER BY Facility
				  DROP TABLE #CANCELLED,#TEMP,#VMSPORTAL
				  END


				  --exec USP_ADMINBUCKETBA
GO
