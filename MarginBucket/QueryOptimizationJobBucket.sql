...
(
	...
	WHERE 1=1
	AND (FP.SubmitStatus IN (411,414,423,426,567,568,576,579,601,602,619,623,638,639,670))
	AND FP.trustaffCompany_ID != 9
	AND fp.RequestDate > GETDATE() - 365
) as x
where 1=1
--and Recruiter  = SUBSTRING('abrooks', 0,9) OR SubmittedBy = 'abrooks' --Add Account Mgr parameter here
and (SUBSTRING(AM,0,9)) = 'JENDRES' OR SubmittedBy = 'JENDRES'
--and (SUBSTRING(Recruiter,0,8) = 'ABROOKS' OR SubmittedBy = 'ABROOKS')
order by Facility
--order by Recruiter
