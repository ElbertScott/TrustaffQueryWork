...
from (
		SELECT fp.FacPacketID, C1.COMPANY AS Facility,
							...
							LEFT JOIN 
							(
								select FacPacketID, InsertDate, DATEDIFF(dd,InsertDate, getdate()) as Age
								from 
								(
									select ReadyToGoID, FacPacketID, InsertDate,
									ROW_NUMBER() OVER(Partition by FacPacketID order by ReadyToGoID) as row
									from [MCSQLTST01].[MarginBucket].dbo.ReadyToGo
								) as x 
								where row = 1
							) as rtgage
								on FP.FacPacketID = rtgage.FacPacketID	
